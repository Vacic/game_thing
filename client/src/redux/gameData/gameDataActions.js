...
export const login = (email, password) => async (dispatch) => {
  ...
  try {
    ...
    dispatch(
      setNotification({ msg: `Logged In Successfully`, classType: "success" })
    );
    return true;
  } catch (err) {
...
export const logout = () => (dispatch) => {
  ...
  dispatch(
    setNotification({ msg: `Logged Out Successfully`, classType: "danger" })
  );
  return true;
};
...
export const populateGame = () => async (dispatch) => {
  ...
  try {
    ...
    dispatch(
      populatePlayer({ playerStats, inventory, equipment, quickBarEquipment })
    );
    ...
    dispatch(setLoading(false));
    dispatch(
      setNotification({ msg: `Save File Loaded`, classType: "success" })
    );
  } catch (err) {
    cookies.remove("loggedIn");
    dispatch(
      setNotification({ msg: `Save File Failed to Load`, classType: "danger" })
    );
    if (err.response && err.response.data && err.response.data.error) {
      dispatch(setLoading(false));
      dispatch(
        setMessage({
          msg: `${err.response.data.error} - Could Not Load Saved Data`,
          classType: "danger",
        })
      );
    } else {
      ...
      dispatch(
        setMessage({
          msg: "Internal Server Error - Could Not Load Saved Data",
          classType: "danger",
        })
      );
    }
