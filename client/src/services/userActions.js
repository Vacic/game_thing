...
import { logout, populatePlayer, setCurrentEnemyHp, setCurrentEnemyStats, setCurrentLocation, setCurrentPlayerHp, setMessage, updateUser, setLogin, setLoading, setNotification } from '../redux';
    ...
    if(cookies.get('loggedIn')) {
        try {
            ...
            if (err.response && err.response.status === 403) {
                dispatch(setMessage({ msg: 'Please Login To Continue', classType: 'danger' }));
                dispatch(logout());
...
export const editProfile = async formData => {
    ...
    try {
        ...
        if (err.response && err.response.data && err.response.data.error) dispatch(setMessage({ msg: err.response.data.error, classType: 'danger' }));
        else {
            console.log(err);
...
export const deleteAccount = async () => {
    try {
        ...
        dispatch(setNotification({ msg: 'Account Successfully Removed', classType: 'success' }));
        return true;
    } catch (err) {
        if (err.response && err.response.data.error) dispatch(setMessage({ msg: err.response.data.error, classType: 'danger' }));
        else {
            console.log(err);
            console.log('Internal Server Error');
        }
