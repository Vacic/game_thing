...
const ConfirmationModal = ({ modal: { text, func, showModal, itemToRemove, history, redirectTo, redirectDelay }, toggleModal }) => {
    useEffect(() => {
    ...
    return (
        <div className={`modal-container ${showModal ? 'show-modal' : 'hide-modal'}`}>
            <div className={`confirmation-modal ${showModal ? '' : 'hide'}`} ref={confirmationModal}>
                <span onClick={() => toggleModal()}>x</span>
                <p className="modal-text">{text}</p>
                <div className="modal-buttons">
                    <div className="btn neutral" onClick={() => runFunc()}>Yes</div>
                    <div className="btn danger" onClick={() => toggleModal()}>No</div>
                </div>
            </div>
        </div>
    )
