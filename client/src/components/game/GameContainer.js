...
class GameContainer extends PureComponent {
  ...
  componentDidMount = async () => {
    this.playerHpBar.style.width = `${Math.floor(
      (this.props.currentPlayerHp / this.props.playerStats.hp) * 100
    )}%`;
    ...
    if (this.props.loggedIn)
      this.updateProgressInterval = setInterval(
        () => updateDbProgress(this.props.history),
        15000
      );
  ...
  initCombat = (location) => {
    ...
    this.loadingTimeout = setTimeout(() => {
      ...
      this.startPlayerAttack(
        this.props.playerStats.dmg,
        this.props.playerStats.attSpd,
        this.props.currentEnemy.def,
        this.props.currentEnemy.eva
      );
      this.startEnemyAttack(
        this.props.currentEnemy.dmg,
        this.props.currentEnemy.attSpd,
        this.props.playerStats.def,
        this.props.playerStats.eva
      );
  ...
  stopCombat = () => {
    this.resetActions();
    this.props.setCurrentEnemyStats({
  ...
  resetPlayerAttack = (playerDmg, playerAttSpd) => {
    // Using this after equipping a new weapon
    clearInterval(this.playerAttInterval);
    this.playerAttProgressDiv.style.animation = "none";
    setTimeout(
      () =>
  ...
  startPlayerAttack = (dmg, attSpd, enemyDef, enemyEva) => {
    ...
    this.playerAttInterval = setInterval(() => {
      // Assigning it to a variable so i can stop the interval in 'resetActions'
  ...
  handleUseItem = (item) => {
    if (item && item.stats.heal) {
      const newHp =
        this.props.currentPlayerHp + item.stats.heal >=
        this.props.playerStats.hp
          ? this.props.playerStats.hp
          : this.props.currentPlayerHp + item.stats.heal;
      this.props.setCurrentPlayerHp(newHp);
      this.playerHpBar.style.width = `${Math.floor(
        (newHp / this.props.playerStats.hp) * 100
      )}%`;
  ...
  startEnemyAttack = (dmg, attSpd, playerDef, playerEva) => {
    ...
    this.enemyAttInterval = setInterval(() => {
      // Assigning it to a variable so i can stop the interval in resetActions
  ...
  enemyTakesDamage = (playerDmg, def, eva) => {
    if (this.shouldAttHit(eva)) {
      ...
      this.enemyHpBar.style.width = `${Math.floor(
        (this.props.currentEnemyHp / this.props.currentEnemy.hp) * 100
      )}%`;
    } else {
      this.enemyAttStatus.classList.remove("dmg");
  ...
  render() {
    return (
      <div className="container">
        <LocationSelection initCombat={this.initCombat} />
        ...
        <ConfirmationModal />
        <InformationModal
          enemyName={this.props.currentEnemy.name}
...
const mapDispatchToProps = (dispatch) => {
  return {
    setCurrentEnemyHp: (currentEnemyHp) =>
      dispatch(setCurrentEnemyHp(currentEnemyHp)),
    setCurrentEnemyStats: (currentEnemy) =>
      dispatch(setCurrentEnemyStats(currentEnemy)),
    setCurrentPlayerHp: (currentPlayerHp) =>
      dispatch(setCurrentPlayerHp(currentPlayerHp)),
...
export default withRouter(
  connect(mapStateToProps, mapDispatchToProps)(GameContainer)
);
