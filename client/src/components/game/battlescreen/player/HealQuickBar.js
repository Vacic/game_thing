...
const HealQuickBar = ({ quickBarEquipment, itemList, inventory, handleUseItem }) => 
    <div className="heal-quick-bar">
        <div className="heal-quick-bar-container">
            {quickBarEquipment.map((itemName, i) =>     
                itemName!=='' ? <QuickBarItem key={i} item={itemList[itemName]} itemCount={inventory[itemName]} handleUseItem={handleUseItem} />
                : <QuickBarItem key={i} />
            )}
        </div>
    </div>
