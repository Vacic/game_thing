...
const InvAndEquip = React.memo((props) => {
    ...
    const equipItem = item => {
        ...
        if (emptySlot || item.name !== newPlayerEquip[item.type].name) { // App crashes if not checking for 'emptySlot' because it tries to read newPlayerEquip[item.type].name
            ...
            let newPlayerStats = unequipItem(newPlayerEquip[item.type], emptySlot, newInventory);
            // Equip the new item
        ...
        } else setNotification({ classType: 'danger', msg:`You have already equipped ${item.name}`, img: item.img });
    }
