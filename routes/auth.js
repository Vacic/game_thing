  ...
  try {
    ...
    jwt.sign(
      ...
      (err, token) => {
        ...
        res.status(200).end();
        // res.status(200).json({ token: `Bearer ${token}` }); // FOR LOCAL DEV TOKEN
      }
    );
  } catch (err) {
    console.log(err);
...
router.get("/check-token", checkToken, async (req, res) => {
  ...
  try {
    if (exp - currentDate < 864000) {
      // If less than 10 days remain before the token expires create a new one - refreshes every fifth day
      jwt.sign(
        { id },
        process.env.JWT_SECRET,
        { expiresIn: "15days" },
        (err, token) => {
          ...
          res.status(200).end();
          // res.status(200).json(`Bearer ${token}`); // FOR LOCAL DEV TOKEN
        }
      );
    } else {
      res.status(200).end();
